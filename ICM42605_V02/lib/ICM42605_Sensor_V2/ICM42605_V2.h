...
class ICM42605{
    public:
        ICM42605(SPIClass &bus,uint8_t csPin,uint8_t APEX);
        ...
        //Get Accel, Gyro, Temp
        void readSensor(int16_t * destination);
        double getAccelX();
        double getAccelY();
        double getAccelZ();
        double getGyroX();
    ...
    private:
        ...
        bool _useTap = false;
        //Private Functions
        int writeAll(uint8_t subAddress, uint8_t data);
        int writeBits(uint8_t subAddress, uint8_t data, uint8_t mask);
